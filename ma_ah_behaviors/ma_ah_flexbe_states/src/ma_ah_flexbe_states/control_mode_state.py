...
class ControlModeState(EventState):
    ...
    def __init__(self):
        ...
        self.sub_moving_state = ProxySubscriberCached({"/control/moving/state", MovingParam})
    ...
    def get_param(self, msg):
        ...
        Logger.loginfo("get : moving_type : %d",self.moving_type)
        Logger.loginfo("get : moving_angluar : %f",self.moving_angluar)
        Logger.loginfo("get : moving_linear : %f", self.moving_linear)

        if self.is_step_left == True:
            Logger.loginfo("now left step stil working")
        elif self.is_step_right == True:
            Logger.loginfo("now right step stil working")
        elif self.is_step_forward == True:
            Logger.loginfo("now forward step stil working")
        elif self.is_step_backward == True:
            Logger.loginfo("now backward step stil working")
